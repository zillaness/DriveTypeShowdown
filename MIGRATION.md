...
- **Current build:** `frc_drive_showdown_v5.1.43.html`
...
5. 📋 **RAMMING tackle should hit ALLIANCE bots too (ball modes)** — today RAM (the arcade boost-tackle, `arcadeTackle`) lands hits in TANK FIGHT (tanks tackle foes) and the boost *dash* speeds a bot up in ball/race, but in 2P BALL/SHOOTER a boosting human bot doesn't actually *tackle* (stun/knock) the alliance/CPU bots it rams. Wire `arcadeTackle` (or an equivalent shove/stun) into `updateP2Ball` so a dash-ram staggers the alliance bots on contact, like it does to enemy tanks. Check interaction with `B2_SHOVE`, PIN PENALTY contact rule, and the existing tank-tank push. (Requested by Sam.)
6. 📋 **RAM should knock the balls out of a carrying CPU bot** — if not already a feature: a successful RAM/tackle on a ball-carrying CPU H2H bot should make it DROP (spill) the balls it's holding (`bot.load`), scattering them onto the field. Pairs with item 5 (RAM hitting alliance bots). Likely lives in the ball-mode tackle hook + `b2` carry/`load` model (`b2Sticky`/`b2StickyCarriers`). (Requested by Sam.)
7. 📋 **RAM cooldown — shorten + make it adjustable** — `BOOST.cd` (currently 1.4s) gates RAM/dash reuse; Sam wants it shorter so you can spam the dash. Either drop the default or (preferred) add a Konami **RAM COOLDOWN** slider cheat (e.g. 0.1×–1×, scaling `BOOST.cd`) so it's tunable; live like the other size/ice sliders. Cheat ⇒ records-disable already handled by `anyCheat()`. (Requested by Sam.)
8. 📋 **Swerve wheels: clearer "all point the same way" + rotation readability** — make the swerve drive's wheel modules read better so you can see (a) all 4 pods steering to the same angle in translation, and (b) how they splay/counter-rotate when the chassis spins. `drawSwerveWhl` (the per-module wheel renderer; housing was removed in v5.1.40, leaving just the rotating wheel + direction arrow). Ideas: bolder/longer direction arrows, a faint angle tick, brighter contrast vs the chassis, maybe a short motion trail on rotate. Visual-only; no physics change. (Requested by Sam.)
9. 📋 **Alliance team ring opacity ↑ for wheel visibility** — bump the red/blue team-circle signifier's opacity so the team read is stronger and the wheels stay clearly visible (`p2AllianceRing`: dashed ring at r=RR+10, currently lineWidth 3 over a dark underlay). Pairs with item 8. (Requested by Sam — confirm direction: a more SOLID ring vs the current dashed look.)
