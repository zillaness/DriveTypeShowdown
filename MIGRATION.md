...
- **Current build:** `frc_drive_showdown_v5.1.45.html`
...
11. 📋 **Replace DRIFT cheat with SUPER SPEED (slider)** — DRIFT is tabled/WIP ("DRIFT (WIP)", the slide-out-on-turns model never worked). Replace it in the Konami CHEAT menu with a **SUPER SPEED** cheat — a slider multiplier on movement speed (e.g. 1×–3×, like the BALL/ROBOT SIZE + ICE SLIP sliders). **Distinct from `sensitivity` but STACKS with it:** `sensitivity` (SENS_MIN 0.3 – SENS_MAX 2.0) already multiplies the input return `{vx,vy,vr}` (lines ~1114/1188), so it's effectively a 0.3×–2× speed lever per player. SUPER SPEED is a SEPARATE global cheat multiplier applied at the SAME return on top of sensitivity, so the two compose (e.g. 2× sens × 3× super = 6× top speed) — they "work together," not redundant. Apply across SP + 2P movement sites; human-only is fine (CPU brains assume baseline speed). Remove/retire the `driftMode`/`MOM.drift` cheat wiring. Cheat ⇒ records-disable via `anyCheat()`. (Requested by Sam.)
12. 📋 **Decouple rotation vs translation via a RATIO knob (keep the simple sensitivity lever)** — today `sensitivity` scales `vx`, `vy`, AND `vr` by one factor (`{vx:vx*sensitivity, vy:vy*sensitivity, vr:vr*sensitivity}`, lines ~1114/1188). Sam: **keep the existing single, simple on-the-fly sensitivity adjustment** (the LB/RB bumper −0.1/+0.1, lines ~1307-1308) as the one master lever — do NOT split it into two equal sliders. Instead add a **rotation : translation RATIO** control in the **PAUSE menu**, default **1:1**, that biases turn-rate vs drive-rate on top of the master sensitivity (e.g. `vr *= sensitivity*rotRatio`, `vx/vy *= sensitivity` — or split around the ratio). So you still bump one sensitivity value live, and the ratio knob (mid-match, in pause) decides how much of it goes to turning vs translating. SUPER SPEED (#11) composes with the translation side. Lower priority / "at some point." (Requested by Sam.)
