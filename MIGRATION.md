...
12. 📋 **Decouple rotational vs translation speed** — today `sensitivity` scales `vx`, `vy`, AND `vr` by one factor (`{vx:vx*sensitivity, vy:vy*sensitivity, vr:vr*sensitivity}`, lines ~1114/1188), so you can't make a bot that turns fast but drives slow (or vice-versa). Split into independent levers — e.g. `transSens` (vx/vy) and `rotSens` (vr), or a separate ROTATION SPEED slider alongside the existing sensitivity — so translation and turn rate tune independently. Touches the claim-screen sens UI (one slider → two) and both input returns; SUPER SPEED (#11) should compose with the translation lever. **Surface the controls in the PAUSE menu** (so you can tune turn vs drive rate mid-match), in addition to / instead of the claim screen. Lower priority / "at some point." (Requested by Sam.)
