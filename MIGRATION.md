...
11. 📋 **Replace DRIFT cheat with SUPER SPEED (slider)** — DRIFT is tabled/WIP ("DRIFT (WIP)", the slide-out-on-turns model never worked). Replace it in the Konami CHEAT menu with a **SUPER SPEED** cheat — a slider that multiplies movement speed (e.g. 1×–3× via a `superSpeed` global applied to `SPD`/`TSP` or the per-bot input, like the BALL/ROBOT SIZE + ICE SLIP sliders). Remove `driftMode`/`MOM.drift` wiring (or leave momentum infra, just drop the cheat). Apply across SP + 2P movement sites; human-only is fine. Cheat ⇒ records-disable via `anyCheat()`. Watch CPU brains (they assume baseline speed) — likely human-only multiplier. (Requested by Sam.)
