...
(function (root) {
  ...
  var TOPICS = {
    ...
    permit:      { label: 'Permit Test (driving)',    reward: null     }   // v5.1.260 driver's-license mini-game
  };
  ...
  var QUESTIONS = [
    ...
    /* ---- PERMIT TEST (driving rules — the license mini-game) ---- */
    { id:'lic01', topic:'permit', level:'core',
      prompt:'A red eight-sided (octagon) sign means…',
      options:['STOP — come to a complete stop','Slow down a little','Yield if busy','No parking'],
      answer:0,
      explain:'A red octagon is always STOP: come to a FULL stop, check it’s clear, then go.',
      src:'permit basics' },
    { id:'lic02', topic:'permit', level:'core',
      prompt:'A downward-pointing triangle (yield) sign means…',
      options:['Slow down and let others go first','Stop completely every time','Speed up to merge','Road closed'],
      answer:0,
      explain:'YIELD: slow down, give the right-of-way to other traffic and pedestrians, and only go when it’s safe.',
      src:'permit basics' },
    { id:'lic03', topic:'permit', level:'core',
      prompt:'At a four-way stop, who goes first?',
      options:['Whoever arrived (stopped) first','The biggest vehicle','Whoever is fastest','Always the car on the left'],
      answer:0,
      explain:'First to stop goes first. If two arrive together, the driver on the RIGHT goes first.',
      src:'permit basics' },
    { id:'lic04', topic:'permit', level:'core',
      prompt:'How much space should you keep behind the car ahead?',
      options:['At least a 3-second following distance','One car length at any speed','As close as possible','Half a second'],
      answer:0,
      explain:'The 3-second rule: pick a fixed point, and you should pass it at least 3 seconds after the car ahead. More in rain/snow.',
      src:'permit basics' },
    { id:'lic05', topic:'permit', level:'core',
      prompt:'A school bus stops ahead with its red lights flashing and stop-arm out. You should…',
      options:['STOP and wait until the lights stop flashing','Slow down and pass carefully','Honk and go around','Speed up to clear it'],
      answer:0,
      explain:'Stop for a school bus with flashing red lights (both directions, unless a divided highway separates you) — kids may be crossing.',
      src:'permit basics' },
    { id:'lic06', topic:'permit', level:'core',
      prompt:'When should you use your turn signal?',
      options:['Before every turn or lane change','Only on the highway','Only if someone is behind you','Never — it’s optional'],
      answer:0,
      explain:'Signal BEFORE you turn or change lanes, every time, so others can predict what you’ll do.',
      src:'permit basics' },
    { id:'lic07', topic:'permit', level:'core',
      prompt:'Before changing lanes you should…',
      options:['Signal, check mirrors AND your blind spot','Just check the mirror','Close your eyes and hope','Speed up first'],
      answer:0,
      explain:'Mirrors don’t show everything — turn your head to check the blind spot before moving over.',
      src:'permit basics' },
    { id:'lic08', topic:'permit', level:'core',
      prompt:'A pedestrian is in a crosswalk. You should…',
      options:['Stop and let them cross','Drive around them','Honk so they hurry','Speed up before they reach you'],
      answer:0,
      explain:'Always yield to pedestrians in a crosswalk — they have the right of way.',
      src:'permit basics' },
    { id:'lic09', topic:'permit', level:'core',
      prompt:'A solid YELLOW line on your side of the road means…',
      options:['No passing — don’t cross to pass','Pass whenever you like','Parking lane','Bike lane'],
      answer:0,
      explain:'A solid yellow line means passing is not allowed on your side. Dashed yellow = passing is allowed when clear.',
      src:'permit basics' },
    { id:'lic10', topic:'permit', level:'core',
      prompt:'Texting while driving is…',
      options:['Dangerous and illegal in most places','Fine at red lights','Okay if you’re quick','Only a problem at night'],
      answer:0,
      explain:'Taking your eyes off the road to text hugely raises crash risk — it’s illegal in most places. Eyes up, phone down.',
      src:'permit basics' },
    { id:'lic11', topic:'permit', level:'core',
      prompt:'When should you turn your headlights on?',
      options:['At dusk/night and in rain, fog or snow','Only on the freeway','Only when it’s fully dark','Never in the daytime'],
      answer:0,
      explain:'Headlights help you SEE and BE SEEN — use them at dawn/dusk/night and any time visibility drops (rain, fog, snow).',
      src:'permit basics' },
    { id:'lic12', topic:'permit', level:'core',
      prompt:'Entering a roundabout, you must…',
      options:['Yield to traffic already in the circle','Stop fully even if empty','Go counter to the arrows','Speed up to merge'],
      answer:0,
      explain:'Yield to vehicles already circulating, then enter when there’s a gap and go the direction of the arrows.',
      src:'permit basics' },
    { id:'lic13', topic:'permit', level:'core',
      prompt:'Everyone in the car should…',
      options:['Wear a seatbelt','Only the driver buckles up','Skip belts on short trips','Buckle only on highways'],
      answer:0,
      explain:'Seatbelts save lives — every passenger, every trip, no matter how short.',
      src:'permit basics' },
    { id:'lic14', topic:'permit', level:'core',
      prompt:'A flashing red traffic light means…',
      options:['Treat it like a STOP sign','Go — it’s just broken','Speed up through it','Yield only'],
      answer:0,
      explain:'A flashing red = STOP: come to a full stop, then proceed when clear (like a stop sign). Flashing yellow = slow down and proceed with caution.',
      src:'permit basics' }
  ];
