...
**🎬 CAREER / STORY MODE — ✅ PLANNED in `CAREER_PLAN.md` (full data model + 6-phase build plan + code anchors).** Original vision (Sam, 2026-06-19): a narrative campaign that threads the existing modes into a progression as the player "grows up": **middle school** = learning to drive → **OBSTACLE RACE**; **high school** seasons = the **BALL** modes, first PUSHER then SHOOTER the next season; **college** = TANK FIGHT framed as "the video game you play in your dorm"; **graduate** = you're now competing in **ROBORUMBLE**. Big feature (new meta-shell: progression state, unlocks, story beats between matches) — captured here so it isn't lost; not scoped yet.
...
- **Mode: ONE mode for the whole bracket; the MAP varies per match** (T4, reuses the generalized map gallery). **Map-pick rule (Sam):** the **HIGHER SEED picks** during the bracket; at the **GRAND FINAL the WINNERS-BRACKET team picks** (earned by not losing). Optional **"open / group discussion" mode** = anyone picks, with a **countdown timer that auto-RANDOMIZES** if they don't agree in time. **BUILD ORDER (Sam): the OPEN-PICK-WITH-TIMER mode FIRST** (simplest — show the gallery with a countdown, auto-random on timeout), then layer on the **higher-seed / winners-bracket** pick rules. **✅ DONE v5.1.236:** the open-pick vote module (`startMapVote`/`drawMapVote`/`mapVotePick`/`mapVoteAutoRandom`, phase `p2mapvote`) — reachable via the 🗳 OPEN PICK gallery button; the tournament passes an `onDone` that starts the match. Higher-seed gating still TODO.
...
- **Build phases — ✅ ALL DONE (v5.1.236–241), Tournament v2 is functionally COMPLETE:** ① FORMAT toggle ✅ → ② double-elim engine + Bo3 GF ✅ → ③ **MANUAL seeding** ✅ (v5.1.241, ▲▼ rank-by-skill; RANDOM/QUALIFIER/MANUAL) → ④ **CAPTAIN-DRAFT screen** ✅ (v5.1.239, `p2tdraft`/`tour.alli`, snake draft from a tiered CPU pool) → ⑤ **3v3 routing** ✅ (v5.1.240, `tourMatchAlly`→`tankSeatsFromClaim` allies) → ⑥ **T4 map vote** ✅ (v5.1.238 wired into `tourStartMatch`; open-pick) → ⑦ T1 registration text-input — ✅ already robust (DOM input overlay). An end-to-end smoke plays a 4-captain alliance tournament to a champion (3v3 throughout).
- **✅ Higher-seed/winners-bracket map-pick rule** (v5.1.242, `tourMapPicker`). **✅ T1 registration text-input** already robust (DOM input overlay, iOS-zoom-safe). **One tabled item (Sam deferred):** per-match lineup swaps (drafted once at registration).

- **STOCK → 6-player FFA** (F4) — ✅ **BUILT + PLAYABLE (v5.1.250–251).** RoboRumble TEAM FORMAT = FFA → 6-seat grid (up to 6 humans/CPUs), each its own faction, last-standing / most-HP wins (`bbIsFFA`/`bbCheckResult` generalized to N factions/`FFA_COLS`/`bbFFASpawn`). Polish TODO (Sam playtest): scoreboard/kill-feed, FFA CPU tuning. ~~scoped ~10h+ (engine hard-wires 2 sides: spawn, win-detection, ~51 touchpoints). Recommend a playtest session, not unattended. No separate format toggle — the lives/timer settings (kept valid by the respawn↔timer rule) pick last-standing vs most-kills.~~
- **Tournament v2** (F3) — ✅ **FUNCTIONALLY COMPLETE (v5.1.236–241).** Format toggle + double-elim + Bo3 GF + captain draft + 3v3 alliance routing + per-match open-pick map vote + manual/random/qualifier seeding + size caps. Remaining: higher-seed map-pick gating, per-match lineup swaps, T1 text-input (all non-blocking).
