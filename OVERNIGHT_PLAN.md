...
1. `git fetch origin dev` and make sure you're on it. Current tip = **`drive_showdown_v5.1.120.html`** (v5.1.120).
...
- ▶ **NEXT (for the continuing window):** P9 game modes (KOTH/CTF/push-ball/domination/sumo/VIP) → ★ final balance re-verify → Tournament v2 update (`frcds_tournament_v2_prd_v1.0.md`). Polish: full-screen map gallery; egg full-paint + achievement screen; pincer/kamikaze 3v3 feel; minibot tuning.
- **State: 23 commits this session (v5.1.99→120 + balance), all green on `dev`. P1 weapons · P2 perks · P4 minibots · P5 synergy · P6 eggs · P7 cheats · **P3 hazards+map-select** · balance = DONE.**
