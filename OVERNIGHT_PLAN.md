...
1. `git fetch origin dev` and make sure you're on it. Current tip = **`drive_showdown_v5.1.124.html`** (v5.1.124).
...
- ▶ **NEXT:** Tournament T1 (alliance-name registration + UI overlap/typing fix) → T2 (3v3 via the 6-seat grid + alliance identities through claim/result/champion) → T4 (per-match map-select). Polish: full-screen map gallery; egg full-paint + achievement screen; P9 CTF/push-ball (needs a ball entity).
- **State: 27 commits this session (v5.1.99→124 + balance), all green on `dev`. P1 weapons · P2 perks · P4 minibots · P5 synergy · P6 eggs · **P7 cheats** · **P3 hazards+map-select** · **P9 modes (KO/sumo/domination/VIP)** · **balance re-verified** · **Tournament T3 (RoboRumble in bracket)** = DONE.**
