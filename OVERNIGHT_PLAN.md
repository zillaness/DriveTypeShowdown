...
- **Build phases (UI-heavy, hands-on):** ① ~~FORMAT setting (single/double)~~ **✅ DONE (already in code: p2tnames BRACKET toggle + `tour.format`)** → ② ~~double-elim engine (losers' bracket + bo3 final)~~ **✅ DONE (`tourBuild`/`tour.gf`/`drawTourBracket`)** → ③ manual seeding + CPU tier auto-fill (only RANDOM + QUALIFIER exist today) → ④ **captain-draft screen** (wire the headless `tourAllianceDraft` into a UI — the headline gap) → ⑤ T2 route bracket matches through the 6-seat grid (today `tourStartMatch` is 1v1) → ⑥ T4 per-match map-select (open-pick vote ✅ built v5.1.236, wire into `tourStartMatch`) → ⑦ T1-hard registration text-input fix.
- **⚠️ DOC-CORRECTION (verified in code v5.1.236):** the bracket ENGINE (both formats + bo3 GF + bracket render + seeding rand/qual + drive policy) is already built & reachable. The REMAINING work is the FRC layer: **captain-draft UI (④), 3v3 routing (⑤), map-vote wiring (⑥), manual seeding (③), format size caps.** `tourAllianceDraft` is tested-but-unwired.
