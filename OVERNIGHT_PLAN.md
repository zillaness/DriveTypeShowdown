...
- **Build phases — ✅ ALL DONE (v5.1.236–241), Tournament v2 is functionally COMPLETE:** ① FORMAT toggle ✅ → ② double-elim engine + Bo3 GF ✅ → ③ **MANUAL seeding** ✅ (v5.1.241, ▲▼ rank-by-skill; RANDOM/QUALIFIER/MANUAL) → ④ **CAPTAIN-DRAFT screen** ✅ (v5.1.239, `p2tdraft`/`tour.alli`, snake draft from a tiered CPU pool) → ⑤ **3v3 routing** ✅ (v5.1.240, `tourMatchAlly`→`tankSeatsFromClaim` allies) → ⑥ **T4 map vote** ✅ (v5.1.238 wired into `tourStartMatch`; open-pick) → ⑦ T1 registration text-input — **still TODO** (minor). An end-to-end smoke plays a 4-captain alliance tournament to a champion (3v3 throughout).
- **Future/tabled (non-blocking):** higher-seed/winners-bracket map-pick GATING (today open-pick); per-match lineup swaps (drafted once at registration); T1 text-input fix.
