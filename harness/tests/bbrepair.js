...
;(function(){
  ...
  // heal-rate sanity: how much HP/s the medic-less torch restores to a hurt ally in front
  const A=mkBot('repair',0,0,FH/2);A.x=300;A.y=300;A.h=0;A.ctl.brain.fire=false;A.firing=false;
  ...
  const h0=ally.hp,m0=ally.mob;for(let i=0;i<60;i++){A.firing=false;bbWeaponFire(1/60);}
  console.log('\\nheal rate (1s on a hurt ally): HP '+h0+'→'+ally.hp.toFixed(0)+' (+'+(ally.hp-h0).toFixed(0)+'/s)  ·  mob '+m0+'→'+ally.mob.toFixed(0)+'  ·  wheel re-welded: '+(!ally.wheels[0].dead));
  console.log('tunables: repairHps='+BB_W.repairHps+'  repairMobps='+BB_W.repairMobps+'  repairReachK='+BB_W.repairReachK+'  (a bot has '+BB.HP+' HP)');
})();
`;
