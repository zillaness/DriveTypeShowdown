...
;(function(){
  ...
  startBall('normal');paused=true;{const labs=pauseItems().map(i=>i.lab.replace(/[^A-Z]/g,''));
   ok('2P pause has 7 nav items incl. SETUP/MATCH SETTINGS/MODE/MAIN MENU (v5.1.272: TURN RATE moved to Settings)',pauseItems().length===7&&labs.indexOf('SETUP')>=0&&labs.indexOf('MATCHSETTINGS')>=0&&labs.indexOf('MODE')>=0&&labs.some(l=>l.indexOf('MENU')>=0)&&labs.indexOf('TURNRATE')<0);}
  ...
  startBall('normal');paused=true;{const it=pauseItems(),ms=it.find(x=>x.lab.indexOf('MATCH')>=0);ms.act();ok('v5.1.272 pause → MATCH SETTINGS goes to the match-settings screen (before robot setup)',phase==='p2settings'&&paused===false);}
  {const sv=phase;phase='menu';ok('SP pause has NO MATCH SETTINGS item (2P-only)',!pauseItems().some(x=>x.lab.indexOf('MATCH')>=0));phase=sv;}
  ...
  {phase='playing';holoMode=false;steerMode=false;obstacleCourse=false;curD=1;Object.keys(keys).forEach(kk=>delete keys[kk]);
   ...
   const sl=settingsRows().find(x=>x.kind==='slider'&&x.label==='Turn Rate'); // v5.1.272 TURN RATE is now a SETTINGS drag-slider, not a pause-menu stepper
   ok('TURN RATE is a SETTINGS slider (rotRatio currently 2 from above)',!!sl&&Math.abs(rotRatio-2)<1e-6);
   sl.set(rotRatio-0.25);ok('TURN RATE slider ◀ nudge steps rotRatio down 0.25',Math.abs(rotRatio-1.75)<1e-6);
   sl.set(99);ok('TURN RATE slider clamps to max 3.00×',rotRatio===3);
   sl.act();ok('TURN RATE reset → 1:1',rotRatio===1);keys['ArrowLeft']=false;}
  ...
  // v5.1.279 experimental-gear gate: the MINELAYER is hidden from the loadout until the EXPERIMENTAL BOTS cheat is on
  {const _e=expBots,_c=cannonWeapon;cannonWeapon=false;expBots=false;
   ...
   const ld={weapon:'wedge',armor:'balanced',perk:'none'},s1=new Set();for(let i=0;i<20;i++){bbCycleField(ld,'weapon',1);s1.add(ld.weapon);}
   ok('MINELAYER hidden from the weapon cycler when EXPERIMENTAL BOTS is off',!s1.has('mine'));
   expBots=true;const ld2={weapon:'wedge',armor:'balanced',perk:'none'},s2=new Set();for(let i=0;i<24;i++){bbCycleField(ld2,'weapon',1);s2.add(ld2.weapon);}
